...
const restoredChatIds = userSessions.loadChatIdsFromFile()
userSessions.restoreSessionsOnStartup(restoredChatIds)

if (tempDownloadsCatalog) {
	fs.promises
		.mkdir(tempDownloadsCatalog, { recursive: true })
		.then(() =>
			console.log(
				`Directory created or already exists: ${tempDownloadsCatalog}`
			)
		)
		.catch((err) =>
			console.error(`Failed to create directory: ${tempDownloadsCatalog}`, err)
		)
}

if (tempCatalog) {
	fs.promises
		.mkdir(tempCatalog, { recursive: true })
		.then(() =>
			console.log(`Directory created or already exists: ${tempCatalog}`)
		)
		.catch((err) =>
			console.error(`Failed to create directory: ${tempCatalog}`, err)
		)
}
...
bot.on('message', async (msg) => {
    ...
    userSessions.addChatIdIfNotExists(msg.chat.id)      
