...
module.exports.initializeUserSettings = async function (chatId) {
  try {
    ...
    const response = await axios.post(`${SERVER_URL}/user-get`, { chat_id: chatId }, { headers })
    ...
    console.log('Error initializing user settings:', error?.response?.data || error.message)
    return null
  }
}
