		...
		if (inputLine.includes("id#")) {
			...
			try {
				await bot.sendMessage(
					id,
					`Дякуємо за звернення, відповідь: \n ${msgtext}`,
					{ parse_mode: "HTML" }
				)
...
module.exports.sendstarterButtons = async function (bot, msg) {
	...
	if (adminIds.includes(String(msg.chat.id))) {
		userButtons.push([
			{ text: "✍ for Reply on demand", callback_data: "0_99" },
		])
	}
...
module.exports.clientsAdminResponseToRequest = async function (bot, msg) {
	await bot.sendMessage(
		msg.chat.id,
		"Введіть <i>id чата для відправки відповіді клієнту </i>\n",
		{ parse_mode: "HTML" }
	)
